...
for pdf_file in pdf_files:
    ...
    try:
        ...
        if os.path.exists(os.path.join(pdf_dir, base_name)):
            ...
            if pic1_files:
                print(f"{pdf_name:45} 页数: {page_count:2}  Picture_1: {len(pic1_files):2}个  状态: ❌仍有图标")
            else:
                print(f"{pdf_name:45} 页数: {page_count: 2}  Picture_1: 0个  状态: ✅图标已删除")
...
print(f"  删除成功率: {total_pic_removed/total_chapters*100:.1f}%")
